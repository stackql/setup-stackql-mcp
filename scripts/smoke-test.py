#!/usr/bin/env python3
"""
Deterministic MCP smoke test for the stackql MCP server. Stdlib only.

Spawns a command that speaks MCP over stdio and exercises the JSON-RPC
handshake plus the github provider in no-auth mode:
...
Usage:
  smoke-test.py --cmd "<command ...>"   test an arbitrary stdio MCP command
                                        (e.g. the binary launched via the
                                        action's emitted mcp-config)
"""
...
def run_command(cmd: list[str]) -> None:
    """Smoke-test an arbitrary command that speaks MCP stdio. The command must
    accept extra stackql flags appended after its own arguments (used to pass
    --auth for the github provider)."""
    ...
    try:
        ...
        client.send(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "setup-stackql-mcp-smoke", "version": "1"},
            },
            id_=1,
        )
